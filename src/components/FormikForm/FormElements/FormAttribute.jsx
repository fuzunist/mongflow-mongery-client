...
const FormAttribute = ({
  ...
  console.log("value in formattr", value);

  const addAttribute = () => {
    const newValue = [
      ...value,
      { attribute_name: "", attribute_id: undefined, values: [] },
    ];
  ...
  const attrTypeChange = (attrType, index) => {
    // Teknik vs Paketlme
    const newValue = [...value];
    newValue[index].packaging = attrType;
    onChange({ target: { name, value: newValue } });
  };
  ...
  const addValue = (attrIndex) => {
    const newValue = [...value];
    newValue[attrIndex].values.push({
      value: "",
      extra_price: "0",
      value_id: undefined,
    }); // Push an object instead of an empty string
    onChange({ target: { name, value: newValue } });
  };
  ...
  const valueChange = (attrIndex, valueIndex, valueName, value_id) => {
    const newValue = [...value];
    if (
      typeof newValue[attrIndex].values[valueIndex] === "string" ||
      !newValue[attrIndex].values[valueIndex]
    ) {
      newValue[attrIndex].values[valueIndex] = {
        value: "",
        extra_price: "0",
  ...
  const delValue = (attrIndex, valueIndex) => {
    const newValue = [...value];
    newValue[attrIndex].values = newValue[attrIndex].values.filter(
      (val, indx) => indx !== valueIndex
    );
    onChange({ target: { name, value: newValue } });
  };
  ...
  const extraPriceChange = (attrIndex, valueIndex, newExtraPrice, value_id) => {
    ...
    if (
      ...
      newValue[attrIndex].values[valueIndex] = {
        value: newValue[attrIndex].values[valueIndex],
        extra_price: "0",
        value_id,
      };
    }
          ...
          >
            <div className="flex gap-2 items-center">
              ...
              <Switch
                ...
                size="large"
                checkedChildren="Paket"
                unCheckedChildren="Teknik"
                defaultChecked={false}
                autoFocus={true}
                value={attr.packaging}
                onChange={(e)=>  attrTypeChange(e, attrIndex)}
              />
            ...
            {attr.values.length > 0 && (
              <hr className="w-full border-input-focusborder-light dark:border-input-focusborder-dark mt-1" />
            )}
